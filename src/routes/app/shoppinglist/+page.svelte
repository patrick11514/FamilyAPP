<script lang="ts">
    ...
    let toBuy = $state(data.data.filter((item) => item.bought_by === null).toSorted((a, b) => b.created_at.getTime() - a.created_at.getTime()));
    let bought = $state(data.data.filter((item) => item.bought_by !== null).toSorted((a, b) => b.bought_at!.getTime() - a.bought_at!.getTime()));
    ...
    {#if userData.logged}
        <div class="border-text flex flex-row gap-2 rounded-md border-2 p-2">
            ...
            <div class="flex w-1/4 items-center justify-center">
                {#if item.image}
                    <a href="/images/{item.image}" target="_blank"><Image name={item.image} alt="" /></a>
                {:else}
                    <Icon name="bi-image" class="text-4xl" />
                {/if}
            </div>
            <div class="flex flex-1 flex-col justify-between gap-2">
                <div class={['flex flex-row justify-between', item.bought_by ? 'line-through' : '']}>
                    <h1 class="text-xl font-bold lg:text-2xl">{item.count}x {item.name}</h1>
                    {#if userData.data.id === item.user_id}
                        <Icon onclick={() => removeItem(item.id)} name="bi-trash-fill" class="text-red-500" />
                    {/if}
                </div>
                {#if item.bought_by === null}
                    <h2 class="text-lg lg:text-xl">Přidal/a: {formatUser(data.users.find((u) => u.id === item.user_id)!)} ({toDate(item.created_at)})</h2>
                {:else}
                    <h2 class="text-lg lg:text-xl">Zakoupil/a: {formatUser(data.users.find((u) => u.id === item.bought_by)!)} ({toDate(item.bought_at!)})</h2>
                {/if}
            </div>
        </div>
    {/if}
...
<div class="mt-2 h-full w-full flex-1">
    <div class="flex items-center justify-between text-2xl 2xl:text-3xl">
        ...
        <Icon onclick={buy} name="bi-check-lg" class={[selected.length > 0 ? 'text-green-500' : 'text-gray-500']} />
    </div>
